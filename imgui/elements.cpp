...
bool elements::tab(const char* name, bool boolean)
{
    ...
    window->DrawList->AddLine(ImVec2(rect.Max.x, rect.Min.y + 3), ImVec2(rect.Max.x, rect.Max.y - 3), ImColor(190 / 255.0f, 66 / 255.0f, 66 / 255.0f, it_anim->second.rect_opacity), 2.0f);
...
bool elements::begin_child_ex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText(bold, 15.0f, parent_window->DC.CursorPos + ImVec2(8, 1), ImColor(190, 66, 66), name);
