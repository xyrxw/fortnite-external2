...
namespace Input
{
	...
	{
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_("win32u")), _("NtUserSendInput"));
		if (!NtUserSendInput_Addr)
		{
			NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_("user32")), _("NtUserSendInput"));
			if (!NtUserSendInput_Addr)
			{
				NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_("user32")), _("SendInput"));
				if (!NtUserSendInput_Addr)
					return FALSE;
			}
		}
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_("win32u")), _("NtUserGetAsyncKeyState"));
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_("user32")), _("NtUserGetAsyncKeyState"));
			if (!NtUserGetAsyncKeyState_Addr)
			{
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_("user32")), _("GetAsyncKeyState"));
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}
